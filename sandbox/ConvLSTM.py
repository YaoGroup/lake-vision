...
# TEMPORAL BLOCK (no attention)
class TemporalBlock(nn.Module):
    def __init__(self, mode='average'):
        """
        Args:
            mode (str): either 'average' or 'last'
        """
        super(TemporalBlock, self).__init__()
        assert mode in ['average', 'last'], "mode must be 'average' or 'last'"
        self.mode = mode
    ...
    def forward(self, x, area_seq=None):
        ...
        Args:
            ...
            area_seq: Ignored (kept for compatibility)
        Returns:
...
class ConvLSTMClassifier(nn.Module):
    def __init__(self,
        ...
        # (8) TEMPORAL BLOCK: [B, T=153, C=32, H=64, W=64] --> [B, C=32, H=64, W=64]
        # self.temporal_block = TemporalBlockAttention(input_channels=lstm2_hidden, use_area=self.use_area) # use this if you want temporal attention
        self.temporal_block = TemporalBlock(mode='last')  # 'last' or 'average' depending on how we want to aggregate the time dimension
    ...
    def forward(self, x, area_seq):
        ...
        x = self.temporal_block(x)    # -> [B, 32, 64, 64]
