...
## ===================== ##
num_epochs = 250
lrs = [1e-3]                       # options: [1e-3, 1e-4]
batch_sizes = [16]                 # options: [8, 16]
optimizers = ["Adam", "SGD"]       # options: ["Adam", "SGD"]
sattn_flags = [0, 1, 2, 3, 4, 5]   # options: [options: (0): none, (1): no cbam yes blurmask as input channel, (2): no cbam yes rgb*blurmask as input channel, (3): yes cbam no blurmask as input channel only rgb, (4): yes cbam yes blurmask as input channel, (5): architectural attention]
imgseq_flags = [True]              # options: [True, False]
areaseq_flags = [False, True]      # options: [True, False]
...
## ========================================== ##
## STRATIFIED TRAIN / VAL SPLIT BY RINES LABEL ##
## ========================================== ##

# 1) ids as numpy
all_ids = ds.ids.to_numpy()

# 2) turn Rines probability vectors -> hard labels via argmax
#    (assumes every row has a valid prob vector)
rines_probs = ds["label_rines"].to_numpy()                 # shape: (N_ids, num_classes)
y = np.argmax(rines_probs, axis=1).astype(int)             # shape: (N_ids,)

# 3) simple stratified split
...
def count_classes(sub_ids, name):
    idx = np.isin(all_ids, sub_ids)
    u, c = np.unique(y[idx], return_counts=True)
    print(f"{name} class counts:", {int(k): int(v) for k, v in zip(u, c)}, flush=True)

overall = {int(k): int(v) for k, v in zip(*np.unique(y, return_counts=True))}
print("Overall class counts:", overall, flush=True)
count_classes(train_ids, "Train")
count_classes(val_ids,   "Val")
...
class ConvLSTMClassifier(nn.Module):
    def __init__(self,
                 ...
                 attn_spatial=0,             # whether (and which) spatial attention to use [options: (0): none, (1): no cbam yes blurmask as input channel, (2): no cbam yes rgb*blurmask as input channels, (3): yes cbam no blurmask as input channel, (4): yes cbam yes blurmask as input channel, (5): architectural attention]
        ...
        if self.attn_spatial in (0, 2, 3, 5): # use only RGB channels (no blurmask) through the frontend CNN
            self.input_channels = 3  # only RGB channels
        else: # only for attn_spatial in (1, 4), where the mask is used as an additional input channel
        ...
        # (2) SPATIAL ATTENTION [options: (0): none, (1): no cbam yes blurmask as input channel, (2): no cbam yes rgb*blurmask as input channel, (3): no cbam no blurmask as input, (4): yes cbam yes blurmask as input channel, (5): architectural attention]
        if self.attn_spatial in (3,4): # use modified CBAM spatial attention
            self.modified_cbam = ModifiedCBAM(in_channels=frontcnn_out_channels)
        elif self.attn_spatial==5:
    ...
    def forward(self, x, area_seq, cloudy_seq):
        ...
        """
        # (1) SHALLOW CNN
        if self.attn_spatial in (0, 3, 5):      # use only RGB channels (no mask) through the frontend CNN (use for no attention, or for cbam with RGB no blurmask input, or for architectural spatial attention)
            ...
            if self.attn_spatial==5:     
                mask_f = self.arch_attn_frontend(mask) # pass just the mask through a frontend CNN for architectural attention
                x = rgb_f * mask_f       # element-wise multiplication -> [B, T, C=16, H=64, W=64] (this is the architectural attention)
                ...
                # print(f"using only RGB features through frontend cnn", flush=True)
        elif self.attn_spatial==1 or self.attn_spatial==4:    # use all four channels (RGB+mask), (mask will be blurred in the batching)
            x = x[:, :, :4, :, :]
            x = self.frontcnn(x)
        ...
        if self.attn_spatial in (3,4):           # use modified CBAM with blurmask as input channel
...
if params["optimizer"] == "Adam":
    ...
    raise ValueError(f"WARNING: unsupported optimizer: {params['optimizer']}. Please use 'Adam' or 'SGD'.")
