...
# path to a real processed data .nc file(s)
PROCESSED_DATA_DIR = Path(__file__).parent.parent.parent / "datasets" / "processed" / "CW2019_1579.nc"

@pytest.mark.skipif(
    not PROCESSED_DATA_DIR.exists() or len(list(PROCESSED_DATA_DIR.glob("*.nc"))) == 0,
...
class TestTrainingPipeline:
    ...
    def test_forward_pass(self):
        ...
        dataset = LakeDataset(PROCESSED_DATA_DIR, seq_len=51, label=0)
    ...
    def test_forward_backward_pass(self):
        ...
        dataset = LakeDataset(PROCESSED_DATA_DIR, seq_len=21, label=0)
        loader = DataLoader(dataset, batch_size=1)
        img_seq, area_seq, label, lake_id = next(iter(loader))
